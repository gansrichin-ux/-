...
class RolePermissions {
  ...
  static bool hasRole(UserModel user, String targetRole) {
    ...
    if (targetRole == carrierForwarder && user.role == 'driver_forwarder') return true;
    if (targetRole == cargoOwnerCarrier && user.role == 'driver_cargo_owner') return true;
  ...
  static bool canUpdateCargoStatus(UserModel user, String? executorId, String cargoOwnerId) {
  ...
  static bool canUploadCargoDocuments(UserModel user, String? executorId, String cargoOwnerId) {
    if (hasRole(user, admin)) return true;
    if (hasRole(user, logistician)) return true;
    if (user.uid == cargoOwnerId) return true;
    if (executorId != null && user.uid == executorId) return true;
